...
var Plugins = function() {
	...
	var initDaterangepicker = function() {
		if ($.fn.daterangepicker) {
			...
			function (start, end) {
				...
				AppTemplate.blockUI($("#content"));
				setTimeout(function () {
					AppTemplate.unblockUI($("#content"));
					noty({
						text: '<strong>Dashboard updated to ' + range_updated + '.</strong>',
						type: 'success',
						timeout: 1000
					});
					//AppTemplate.scrollTo();
				}, 1000);
	...
	var initSparklines = function() {
		if ($.fn.sparkline) {
			...
			$.extend(true, $.fn.sparkline.defaults, {
				line: {
					highlightSpotColor: AppTemplate.getLayoutColorCode('green'),
					highlightLineColor: AppTemplate.getLayoutColorCode('red')
				},
				bar: {
					barColor: AppTemplate.getLayoutColorCode('blue'),
					negBarColor: AppTemplate.getLayoutColorCode('red'),
					barWidth: 5,
					barSpacing: 2
				},
				tristate: {
					posBarColor: AppTemplate.getLayoutColorCode('green'),
					negBarColor: AppTemplate.getLayoutColorCode('red')
				},
				box: {
					medianColor: AppTemplate.getLayoutColorCode('red')
				}
			});
	...
	var defaultPlotOptions = {
		colors: [AppTemplate.getLayoutColorCode('blue'), AppTemplate.getLayoutColorCode('red'), AppTemplate.getLayoutColorCode('green'), AppTemplate.getLayoutColorCode('purple'), AppTemplate.getLayoutColorCode('grey'), AppTemplate.getLayoutColorCode('yellow')],
		legend: {
			show: true,
			...
			$('.knob').each(function () {
				if (typeof $(this).attr('data-fgColor') == 'undefined') {
					$(this).trigger('configure', {
						'fgColor': AppTemplate.getLayoutColorCode('blue'),
						'inputColor': AppTemplate.getLayoutColorCode('blue')
					});
				}
			});
		}
