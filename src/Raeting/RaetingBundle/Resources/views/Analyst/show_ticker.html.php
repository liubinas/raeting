...
<script>
    $(document).ready(function(){
            ...
            var d1 = [<? $total = count($analysisForGraph); for($i=1; $i<$total;$i++): echo '["'.(strtotime($analysisForGraph[$i]->getDate()->format('Y-m-d'))*1000).'","'.$analysisForGraph[$i]->getEstimation().'"]'; if($i != $total-1) echo ', '; endfor; ?>];
...
<?= $view->render('RaetingRaetingBundle::Analyst/analysis_list.html.php', array(
    'analysis' => $analysis, 'searchLink' => 'analyst_show', 'analystId' => $analyst->getId(), 'totalAnalysis' => $totalAnalysis, 'page' => $page, 'perPage' => $perPage)); ?>
