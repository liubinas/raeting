...
class Rate
{
    ...
    public function importStockPrices($data)
    {
        $result = 0;
        $conn = $this->em->getConnection();
        $table = 'symbol_' . strtolower($data['symbol']);
        $query = sprintf(
            "INSERT INTO `%s` (
                `symbol_id`, `bid`, `ask`, `high`, `low`, `created`, `source_time`, `source_date`
             )
             VALUES (:symbolId, :bid, :ask, :high, :low, NOW(), :time, :date)
        ...
        foreach ($data as $row) {
            $stmt->bindValue(":symbolId", $row['symbol_id'], \PDO::PARAM_INT);
            $stmt->bindValue(":bid", $row['bid']);
            $stmt->bindValue(":ask", $row['ask']);
            $stmt->bindValue(":high", $row['high']);
            $stmt->bindValue(":low", $row['low']);

            $stmt->bindValue(":date", $row['date']);
            $stmt->bindValue(":time", $row['time']);
